...
public class Bunker implements GameObject, Renderable, Cloneable {
    ...
    public Bunker clone() {
        try {
            Bunker clonedBunker = (Bunker) super.clone();
            clonedBunker.position = this.position.clone();
            clonedBunker.state = this.state.clone();
            return clonedBunker;
        } catch (CloneNotSupportedException e) {
            throw new AssertionError("Bunker cloning not supported", e);
        }
